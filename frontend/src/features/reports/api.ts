...
export async function listClaimReports(params: ClaimReportParams = {}) {
  const response = await apiClient.get<ClaimReportListResponse>('/reports/claims', { params });
  return response.data;
}

export async function getClaimReportSummary(params: ClaimReportParams = {}) {
  const response = await apiClient.get<ClaimReportSummary>('/reports/claims/summary', { params });
  return response.data;
}
